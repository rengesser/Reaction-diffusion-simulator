...
Y1=NaN(maxid,re.PDE.xmax*re.PDE.ymax,nPlots);

for i = 1:nY
    Y(:,:,i)=re.PDE.Y(:,re.PDE.ctr+i-1);  % to do: reshape to 3 dim
    Y1(:,:,i)=re.PDE.Yar(:,re.PDE.ctr+i-1);  % to do: reshape to 3 dim
end
for i = (nY+1):nPlots
    Y(:,:,i) = repmat(re.px(:,i-nY)',maxid,1);
    Y1(:,:,i) = repmat(re.px(:,i-nY)',maxid,1);
end
    ...
    for idplot = 1:nPlots
        ...
        switch kindofplot
            ...
            case '1dim'
                plot(Y(idT,:,idplot)')
                hold on 
                plot(Y1(idT,:,idplot)','--r')
                hold off
                labelx = 'space';
                labely = 'concentration';
                timestr = [': T=' num2str(re.PDE.t(idT))];
                if idplot == nPlots
                    legend('Simulated with reFramework','Simulated with D2D framework')
                end
        end
